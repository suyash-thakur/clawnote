...
function render() {
  ...
  container.innerHTML = `
    <div class="file-tree-header">
      ...
      <button class="claude-btn" title="Open in Claude Code (${navigator.platform.includes('Mac') ? '\u2318' : 'Ctrl'}+\u21E7+C)">
  ...
  container.querySelector('.claude-btn')?.addEventListener('click', () => {
    const { currentDir: dir } = getState();
    if (!dir) return;
    // Dispatch to the menu handler which manages the terminal panel
    window.dispatchEvent(new CustomEvent('claude:open-request'));
  });
